	...
	<div class ="cart">
		...
		<div class="cart-content">
			<div class="cart-content-main" v-for="(item, index) in cartList">
				<div class="cart-info">
					<img v-bind:src="productDictList[item.id].image">
				...
				<div class="cart-delete">
					<span class="cart-control-delete" v-on:click="handleDelete(index)">删除</span>
				</div>
			...
			<div class="cart-promotion" v-show="cartList.length">
					...
					<span class="cart-control-promotion" v-on:click="handleSearch">查询</span>
			</div>
			<div class="cart-footer" v-show="cartList.length">
				<div class="cart-footer-desc">
					找到的结果个数:<span>{{ result }}</span>
				</div>
			</div>
...
<script>
	...
	export default{
		...
		methods:{
			handleDelete(index){
				Bus.$emit('deleteCart', SS.clist[index].id);
			},
			handleSearch(){
				if(this.name === ''){
					window.alert('请输入名字');
					...
					$.ajax.get('search/'+this.name).then(res => {
						this.result = res.num
					});		
				}
